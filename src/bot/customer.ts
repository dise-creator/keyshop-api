...
export const initCustomerBot = () => {
  ...
  customerBot.onText(/\/orders/, async (msg: Message) => {
    ...
    try {
      ...
      if (!user) {
        await customerBot.sendMessage(chatId,
          '❌ Telegram не привязан. Отправь свой email для привязки.'
        );
        return;
      }
  ...
  // Обработка отзывов (callback от inline кнопок)
  customerBot.on('callback_query', async (query: CallbackQuery) => {
    ...
    if (data.startsWith('rating:')) {
      ...
      try {
        ...
        // Сохраняем рейтинг, просим комментарий
        ...
        // Ждём комментарий — помечаем в памяти
  ...
  customerBot.on('message', async (msg: Message) => {
    ...
    if (!text || text.startsWith('/')) return;
    if (text.includes('@')) return; // это email, уже обработан выше
    ...
    try {
      ...
      await customerBot.sendMessage(chatId,
        '💬 Комментарий сохранён! Спасибо — это очень помогает нам становиться лучше. 🙏'
      );
    } catch (err) {
...
// Временное хранилище ожидающих комментариев: chatId → orderId
const pendingComments = new Map<number, string>();

// Отправить ключ покупателю в Telegram
...
// Уведомить покупателя когда pending → completed
